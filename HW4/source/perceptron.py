...
class Perceptron:
    ...
    def fit(self, X, y, coef_init=None, verbose=False):
        ...
        while mistake_count != 0:
            for i in range(n):
                # Prediction:
                y_prime = self.predict(X[i])
                if y_prime != y[i]:
                    # Update:
                    self.coef_ = self.coef_ + np.dot(X[i], y[i])
                    self.mistakes_[i] += 1
                    if verbose:
                        print("Updated weights: ", self.coef_)
                    mistake_count = n
                else:
                    mistake_count -= 1
                if mistake_count == 0:
...
def main():
    ...
    clf.fit(train_data.X, train_data.y, coef_init = [0, 0], verbose = False)
    print(f'\tcoef = {clf.coef_}')

    clf.fit(train_data.X, train_data.y, coef_init = [1, 0], verbose = False)
    print(f'\tcoef = {clf.coef_}')
    ### ========== TODO: END ========== ###
